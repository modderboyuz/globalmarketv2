...
async function showCategoryProducts(chatId: number, categorySlug: string, page = 1): Promise<void> {
  try {
    const limit = 10; // Max products per page
    const offset = (page - 1) * limit;

    // Step 1: Get category details by its slug
    console.log(`🔍 Fetching category with slug: ${categorySlug}`);
    const { data: category, error: categoryError } = await supabase
      .from("categories")
      .select("id, name_uz, icon, is_active") // Select necessary fields, including icon and active status
      .eq("slug", categorySlug) // Filter by slug
      .single(); // Expecting a single category result

    if (categoryError) {
      console.error(`🔴 Error fetching category by slug '${categorySlug}':`, categoryError);
      throw categoryError; // Throw error to be caught by the outer catch block
    }

    // Check if category was found and is active
    ...
    // Step 2: Get products belonging to this category with specified criteria
    const { data: products, error } = await supabase
      ...
      .select(`
        ...
        stock_quantity,
        average_rating,
        view_count,    -- Ensure this column exists in your 'products' table
        order_count,
        like_count,    -- Ensure this column exists in your 'products' table
        image_url,
        categories:category_id (name_uz, icon), -- Nested select for category details
        sellers:seller_id (username)            -- Nested select for seller username
      `)
      .eq("category_id", category.id)      // Filter by the found category's ID
      ...
      .order("order_count", { ascending: false }) // Order by order count (descending) or popularity_score if it exists
      .range(offset, offset + limit - 1);     // Apply pagination (LIMIT and OFFSET)
    ...
    if (!products || products.length === 0) {
      ...
      // Send message indicating no products found
      await sendTelegramMessage(
        chatId,
        `❌ "${category.name_uz}" kategoriyasida mahsulotlar topilmadi.`,
    ...
    // Step 3: Construct the message and keyboard for the user
    const message = `📦 *${category.icon || ''} ${category.name_uz}*\n\nSahifa ${page} (${products.length} ta mahsulot):\n\nMahsulotni tanlang:`;

    // Pagination keyboard logic
    const paginationKeyboard = [
      // Show "Previous" and "Next" if more than one page exists
      ...(products.length === limit ? [[ // If we fetched the maximum limit, "Next" might be available
        { text: "⬅️ Oldingi", callback_data: `page_${categorySlug}_${Math.max(1, page - 1)}` }, // Previous page button
        { text: "➡️ Keyingi", callback_data: `page_${categorySlug}_${page + 1}` },         // Next page button
      ...
      ]] : []), // If it's the first page and less than limit, no pagination buttons needed
      [{ text: "🔙 Kategoriyalarga qaytish", callback_data: "categories" }], // Always provide a way back to categories
    ];
    ...
    const productKeyboard = products.map(product => [
      {
        ...
        callback_data: `product_${product.id}`, // Callback data to get product details
      },
    ]);

    // Combine product buttons and pagination buttons
    const keyboard = {
      inline_keyboard: [...productKeyboard, ...paginationKeyboard],
    };
    ...
    await sendTelegramMessage(
      chatId,
      message,
      { reply_markup: keyboard }, // Ensure reply_markup is correctly structured
      "Markdown"
    );

  } catch (error: any) { // Catch any errors that occurred during the process

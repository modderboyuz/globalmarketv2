...
bot.onText(/\/orders/, async (msg) => {
  ...
  // Endi faqat SPECIFIC_ADMIN_TELEGRAM_ID ga ega bo'lgan foydalanuvchi admin hisoblanadi.
  const isAdmin = await checkAdminStatus(userId);
...
bot.on("callback_query", async (callbackQuery) => {
  ...
  // Agar admin bo'lmasa va bu adminlik talab qiluvchi tugma bo'lsa, xabar beramiz.
  ...
  try {
    ...
    if (!existingUser) {
      const { error } = await supabase.from("users").insert({
        ...
        // Agar sizda boshqa joylarda ham foydalanuvchi ma'lumotlari bo'lsa,
        // ularni shu yerga qo'shib olishingiz mumkin.
        // Masalan: email, username va hokazo.
        email: `telegram_${telegramId}@temp.com`, // Agar email kerak bo'lsa
      });
...
// Endi faqat SPECIFIC_ADMIN_TELEGRAM_ID ni tekshiradi, bazadan emas.
    ...
    const keyboard = {
      reply_markup: {
        inline_keyboard: [
          ...
          // Bu tugma uchun callback data handle qilinmayapti, shuning uchun o'chirish yoki qo'shish kerak
    ...
    const keyboard = {
      reply_markup: {
        inline_keyboard: [
          ...
          [{ text: "📞 Qo'ng'iroq qilish", url: "tel:+998901234567" }], // Telefon raqamni o'zgartirishingiz mumkin
        ],
      },
    };
...
// Bu funksiya endi faqatgina SPECIFIC_ADMIN_TELEGRAM_ID ga xabar yuboradi.
